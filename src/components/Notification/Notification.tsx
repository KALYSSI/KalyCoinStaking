    ...
    <div className={s.notify}>
      ...
      <p className={s.notify_desc}>
        ...
        {transactionStatus === "stake_loading" && !status && (
          <>
            Adding <span className={s.notify_accent}>{tokenAmount}</span> KLC to Staking
          </>
        )}
        {transactionStatus === "withdraw_loading" && (
          <>
            Withdrawing <span className={s.notify_accent}>{tokenAmount}</span> KLC
          </>
        )}
        ...
        {transactionStatus === "claim_loading" && (
          <>
            Adding <span className={s.notify_accent}>{tokenAmount}</span> KLC to wallet
          </>
        )}
        ...
        {status === "success_stake" && (
          <>
            <span className={s.notify_accent}>{tokenAmount} KLC </span> successfully{" "}
            <span className={s.notify_string}>added to Staking</span>
          </>
        )}
        {status === "success_withdraw" && (
          <>
            <span className={s.notify_accent}>{tokenAmount} KLC </span>
